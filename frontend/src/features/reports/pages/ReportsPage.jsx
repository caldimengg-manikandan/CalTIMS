    ...
    return (
        <div className="mb-4">
            <div className="flex justify-between text-xs font-semibold mb-1.5">
                ...
                <span className="text-slate-800 dark:text-white">{value.toFixed(1)}h <span className="text-slate-400 font-normal">/ {max}h</span></span>
            </div>
...
export default function ReportsPage() {
    ...
    if (appVersion === 'basic') {
        return (
            <div className="flex flex-col items-center justify-center h-[70vh] text-center animate-fade-in">
                ...
                <h2 className="text-2xl font-bold text-slate-800 dark:text-white mb-3">Enterprise Analytics</h2>
                <p className="text-slate-500 max-w-md mb-8">
                    Unlock advanced reporting, compliance tracking, AI-powered insights, and department utilization metrics with the Pro tier.
                </p>
                <button className="btn btn-primary px-8">Upgrade to Pro</button>
            </div>
        )
    }
    ...
    const deptChartData = (deptData || []).map(d => {
        const row = { name: d.department || 'Unassigned', total: +(d.totalHours || 0).toFixed(1) }
    ...
    return (
        <div className="space-y-6 animate-fade-in pb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                <KpiCard icon={Clock} label="Total Hours Logged" value={`${totalHours.toFixed(1)}h`}
                                ...
                                {topPerformers.length ? topPerformers.map((emp, idx) => (
                                    <div key={emp._id} className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 hover:border-pink-200 transition-colors">
                                        ...
                                        <div className="text-right">
                                            <p className="text-sm font-black text-pink-500">{emp.hours.toFixed(1)}h</p>
                                        </div>
                                    </div>
                                        ...
                                        return (
                                            <tr key={i} className="hover:bg-slate-50/80 dark:hover:bg-white/5 transition-colors group">
                                                ...
                                                <td className="py-3 px-4 text-right">
                                                    <span className="font-black text-cyan-600 dark:text-cyan-400 text-base">{row.totalHours?.toFixed(1)}</span><span className="text-slate-400 text-xs ml-0.5">h</span>
                                                </td>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-white/5">
                        ...
                        <div className="text-left sm:text-right bg-white dark:bg-slate-800 p-3 rounded-lg shadow-sm border border-slate-100 dark:border-slate-700 min-w-[120px]">
                            ...
                            <p className="text-2xl font-black text-cyan-600 leading-none">
                                {taskDetails?.reduce((s, t) => s + t.hoursWorked, 0).toFixed(1)}<span className="text-sm ml-1 text-slate-400">hours</span>
                            </p>
                        </div>
                    </div>
