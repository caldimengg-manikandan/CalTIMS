        ...
        <div className="space-y-8 pb-10">
            <div className="flex items-center justify-between">
                ...
                <button
                    ...
                    className="group flex items-center gap-2 px-5 py-2.5 rounded-2xl btn-primary text-white shadow-sm"
                >
            ...
            <div className="grid grid-cols-1 gap-6">
                <SectionCard title="Permission Profiles" subtitle="Configure system and custom roles" icon={Shield}>
                    <div className="space-y-6">
                        {roles.map((role, idx) => (
                            <div key={idx} className="p-6 rounded-3xl border border-slate-200 dark:border-white/10 bg-slate-50/50 dark:bg-black/20 hover:border-primary0/30 transition-colors group">
                                <div className="flex flex-col lg:flex-row gap-8 items-start">
                                    <div className="w-full lg:w-1/4">
                                        <div className="flex items-center gap-3 mb-1">
                                            <label className="text-[10px] font-black uppercase tracking-widest text-slate-500">Role Title</label>
                                            {role.isSystem && (
                                                <span className="px-1.5 py-0.5 rounded-md bg-slate-200 dark:bg-white/10 text-slate-500 dark:text-slate-500 text-[8px] font-black uppercase">Core System</span>
                                            )}
                                        </div>
                                        <input
                                            value={role.name}
                                    ...
                                    <div className="flex-1">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-3 block">Granular Permissions</label>
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        className={`flex items-center gap-3 p-3 rounded-2xl border transition-all text-left ${isActive
                                                            ? 'btn-primary border-primary text-white shadow-lg shadow-primary/20'
                                                            : 'bg-white dark:bg-white/5 border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-500 hover:border-primary/50'
                                                    ...
                                                    >
                                                        <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${isActive ? 'bg-white border-white' : 'border-slate-300 dark:border-slate-700'}`}>
                                                            {isActive && <div className="w-1.5 h-1.5 rounded-full bg-primary shadow-sm"></div>}
                                                        </div>
